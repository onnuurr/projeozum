...
.search-box input { padding: 8px 12px; border: 1px solid rgb(var(--portal-border)); border-radius: 8px; font-size: 13px; min-width: 280px; }
.btn-ghost { padding: 8px 16px; border-radius: 8px; background: rgb(var(--portal-bg)); border: 1px solid rgb(var(--portal-border)); cursor: pointer; font-size: 13px; }
.filter-bar { display: flex; gap: 8px; margin-bottom: 16px; flex-wrap: wrap; align-items: center; }
.filter-select { padding: 8px 12px; border: 1px solid rgb(var(--portal-border)); border-radius: 8px; font-size: 13px; background: #fff; color: rgb(var(--portal-ink)); cursor: pointer; min-width: 160px; }
.grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 16px; }
.product-card { background: #fff; border-radius: 12px; border: 1px solid rgb(var(--portal-border)); overflow: hidden; text-decoration: none; color: inherit; transition: box-shadow .15s, transform .15s; }
.product-card:hover { box-shadow: 0 6px 16px rgba(17,24,39,.08); transform: translateY(-1px); }
.product-img { aspect-ratio: 3/4; background-size: cover; background-position: center; background-color: rgb(var(--portal-bg)); }
.product-info { padding: 10px 12px; display: flex; flex-direction: column; gap: 4px; }
.brand { font-size: 10px; color: rgb(var(--portal-text-muted)); text-transform: uppercase; letter-spacing: 0.04em; }
.name { font-size: 13px; font-weight: 600; color: rgb(var(--portal-ink)); line-height: 1.3; }
.prices { display: flex; justify-content: space-between; align-items: baseline; margin-top: 6px; }
.tenant-price { font-size: 15px; font-weight: 700; color: rgb(var(--portal-accent)); font-family: 'SF Mono', Menlo, Consolas, monospace; }
.purchase-price { font-size: 10px; color: rgb(var(--portal-text-muted)); }
.empty { text-align: center; padding: 64px; color: rgb(var(--portal-text-muted)); }
