 ...
 *
 * Fixture path: Modules/Tenant/Resources/fixtures/marketplace/trendyol/{orders,products}.json
 * README: fixture provider doc version'ını pinler (rot önleme).
 */
class TrendyolStubService extends AbstractMarketplaceService
{
    ...
    public function pushProduct(Product $product): PushResult
    {
        ...
        return new PushResult(
            success: true,
            pushed: max(1, $product->variants()->count()),
            externalIds: [(string) $product->id => 'STUB-BATCH-' . uniqid()],
        );
    }
    ...
    {
        $path = __DIR__ . '/../../../Resources/fixtures/marketplace/trendyol/' . $file;
        if (! is_file($path)) {
            return [];
        }
