...
/**
 * Tenant'a özgü persistence çekirdeği. Eskiden AbstractMarketplaceService'in parçasıydı
 * (her provider client'ı bunu extend ediyordu); artık Modules\Marketplace saf bir API
 * entegrasyon motoru olduğu için bu sorumluluk burada, stateless bir servis olarak yaşıyor.
 *
 * Burada olan: marketplace_sales upsert, marketplace_expenses kayıt, sync_log lifecycle,
 * commission lookup (category-specific → default fallback), sku→productId eşlemesi.
 */
class TenantMarketplaceSyncService
{
    /**
     ...
     * productId DTO'da null geldiyse (Marketplace modülü hiç çözmez) sku üzerinden aranır.
     */
    public function recordSale(Tenant $tenant, MarketplaceOrderDTO $order, MarketplaceOrderLineDTO $line): MarketplaceSale
    {
        return DB::transaction(function () use ($tenant, $order, $line) {
            $productId = $line->productId;
            if ($productId === null && $line->sku !== null && $line->sku !== '') {
                $resolved = Product::query()->where('sku', $line->sku)->value('id');
                $productId = $resolved !== null ? (int) $resolved : null;
            }
            ...
            if ($commission <= 0 && $productId !== null) {
     ...
     *   $log = $sync->startSyncLog($tenant, 'trendyol', 'pull_orders');
     *   try { ... $sync->finishSyncLog($log, ['items_processed' => $n]); }
     *   catch (\Throwable $e) { $sync->failSyncLog($log, $e->getMessage()); throw $e; }
     */
    public function startSyncLog(Tenant $tenant, string $marketplace, string $operation): MarketplaceSyncLog
    {
     ...
     */
    public function lookupRates(string $marketplace, int $categoryId): array
    {
