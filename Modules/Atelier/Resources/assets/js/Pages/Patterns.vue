<template>
	...
	<div class="page-patterns">
		...
		<PageHeader title="Kalıp Kütüphanesi">
			<template #subtitle><strong>{{ patterns.length }}</strong> kalıp · aranabilir DXF arşivi</template>
			<template v-if="canManage" #actions>
				<Button variant="ghost" @click="importModal = true">PDF'ten ekle</Button>
		...
		<div class="filter-bar card">
			...
			<Button v-if="hasFilters" variant="ghost" size="sm" @click="clearFilters">Temizle</Button>
		</div>
		...
		<EmptyState
			...
			:icon="Ruler"
			:title="hasFilters ? 'Filtreye uyan kalıp yok.' : 'Henüz kalıp yok.'"
			:hint="!hasFilters && canManage ? '“Yeni Kalıp” ile arşive ilk kaydı ekle.' : ''"
			class="lib-empty card"
		/>
			...
			<article v-for="p in patterns" :key="p.id" class="pattern-card">
				...
				<div class="pc-body">
					...
					<div class="pc-tags">
						...
						<span v-if="p.scaleVerified" class="scale-ok" title="Ölçek doğrulandı"><Check :size="10" /> ölçek</span>
					...
					<div class="pc-foot">
						...
						<div v-if="canManage" class="pc-actions">
							<button v-if="p.extractionStatus === 'failed'" class="table-action-btn" title="Çıkarımı yeniden dene" @click="retryExtraction(p)"><RotateCw :size="13" /></button>
							<button v-if="p.extractionStatus === 'needs_tracing'" class="table-action-btn" title="Sayısallaştır (elle izle)" @click="openTracer(p)"><PenLine :size="13" /></button>
							<button class="table-action-btn" title="Düzenle" @click="openEdit(p)"><Pencil :size="13" /></button>
							<button class="table-action-btn danger" title="Sil" @click="remove(p)"><Trash2 :size="13" /></button>
						</div>
					</div>
				</div>
			</article>
		...
		<!-- Ekle / Düzenle modalı -->
		<AppModal v-model="modal" :title="editingId ? 'Kalıp Düzenle' : 'Yeni Kalıp'" size="md">
				<form id="pattern-form" @submit.prevent="submit" class="pattern-form">
					<div class="form-2col">
					...
					<div class="section-block">
						...
						<div v-for="(pt, i) in form.parts" :key="i" class="part-row">
							...
							<button type="button" class="row-del" @click="form.parts.splice(i, 1)"><X :size="14" /></button>
						</div>
					</div>
			...
			<template #footer="{ close }">
				<Button variant="ghost" :disabled="form.processing" @click="close">Vazgeç</Button>
				<Button type="submit" form="pattern-form" variant="primary" :loading="form.processing">
		...
		<!-- PDF içe aktarma -->
		<AppModal v-model="importModal" title="PDF'ten kalıp ekle" size="sm">
			...
			<div v-if="importForm.files.length" class="import-tray">
				<span v-for="(f, i) in importForm.files" :key="i" class="file-chip">
					{{ f.name }} <em @click="importForm.files.splice(i, 1)"><X :size="11" /></em>
				</span>
			</div>
			<span v-if="importForm.errors['files.0']" class="form-error">{{ importForm.errors['files.0'] }}</span>
			<template #footer="{ close }">
				<Button variant="ghost" :disabled="importForm.processing" @click="close">Vazgeç</Button>
				<Button variant="primary" :disabled="!importForm.files.length" :loading="importForm.processing" @click="submitImport">
					{{ importForm.files.length }} PDF gönder
				</Button>
			</template>
		</AppModal>
...
import { Plus, Ruler, Check, RotateCw, PenLine, Pencil, Trash2, X } from 'lucide-vue-next'
...
.scale-ok { display: inline-flex; align-items: center; gap: 2px; font-size: 10.5px; font-weight: 600; color: #059669; }
...
.tag-chip { font-size: 10.5px; color: var(--color-primary); background: var(--color-primary-soft); padding: 2px 7px; border-radius: 5px; }
...
.table-action-btn { display: inline-flex; align-items: center; background: #f3f4f6; border: none; cursor: pointer; padding: 5px 8px; border-radius: 6px; transition: all .15s; }
.table-action-btn:hover { background: var(--color-primary-soft); }
...
.row-del { display: inline-flex; align-items: center; justify-content: center; background: #f3f4f6; border: none; border-radius: 6px; cursor: pointer; color: #999; height: 34px; transition: all .15s; }
...
.tag-chip.removable em { display: inline-flex; align-items: center; font-style: normal; opacity: .6; }
...
.import-drop.dragging { border-color: var(--color-primary); background: var(--color-primary-soft); }
