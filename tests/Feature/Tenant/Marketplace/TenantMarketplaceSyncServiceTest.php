...
class TenantMarketplaceSyncServiceTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_record_sale_falls_back_to_commission_lookup_when_dto_missing(): void
    {
        ...
        $sale = $sync->recordSale($tenant, $order, $line);
    ...
    public function test_sync_log_lifecycle(): void
    {
        ...
        $sync->finishSyncLog($log, ['items_processed' => 5]);
        $log->refresh();
        $this->assertSame('success', $log->status);
        $this->assertSame(5, (int) $log->items_processed);
        $this->assertNotNull($log->finished_at);

        $log2 = $sync->startSyncLog($tenant, 'trendyol', 'webhook');
        $sync->failSyncLog($log2, 'HMAC mismatch');
        $log2->refresh();
